...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($title) || empty($start_datetime) || empty($end_datetime)) {
        ...
        $stmt = $conn->prepare("UPDATE events SET title = ?, description = ?, event_type = ?, online_link = ?, location_address = ?, location_city = ?, location_state = ?, location_pincode = ?, start_datetime = ?, end_datetime = ? WHERE id = ?");
        $stmt->bind_param("ssssssssssi", $title, $description, $event_type, $online_link, $location_address, $location_city, $location_state, $location_pincode, $start_datetime, $end_datetime, $event_id);
...
<head>
    ...
    <title>Edit Event | Kind Souls</title>
...
<body>
    <div class="container">
        <h1><i class="fas fa-calendar-edit"></i> Edit Event</h1>
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <div class="radio-group">
                    <label class="radio-option">
                        <input type="radio" id="online" name="event_type" value="online" <?php echo $event['event_type'] == 'online' ? 'checked' : ''; ?> required>
                        <span>Online Event</span>
                    </label>
                    <label class="radio-option">
                        <input type="radio" id="offline" name="event_type" value="offline" <?php echo $event['event_type'] == 'offline' ? 'checked' : ''; ?>>
                        <span>In-Person Event</span>
                    </label>
                </div>
            </div>
            
            <div id="online-fields" style="display: <?php echo $event['event_type'] == 'online' ? 'block' : 'none'; ?>;">
                <div class="form-group">
                    <label for="online_link">Meeting Link</label>
                    <input type="url" id="online_link" name="online_link" placeholder="https://meet.google.com/..." value="<?php echo htmlspecialchars($event['online_link']); ?>">
                </div>
            </div>
            
            <div id="offline-fields" style="display: <?php echo $event['event_type'] == 'offline' ? 'block' : 'none'; ?>;">
                <div class="form-group">
                    <label for="location_address">Address</label>
                    <input type="text" id="location_address" name="location_address" placeholder="123 Main St" value="<?php echo htmlspecialchars($event['location_address']); ?>">
                </div>
                <div class="address-fields">
                    <div class="form-group">
                        <label for="location_city">City</label>
                        <input type="text" id="location_city" name="location_city" placeholder="New York" value="<?php echo htmlspecialchars($event['location_city']); ?>">
                    </div>
                    <div class="form-group">
                        <label for="location_state">State</label>
                        <input type="text" id="location_state" name="location_state" placeholder="NY" value="<?php echo htmlspecialchars($event['location_state']); ?>">
                    </div>
                    <div class="form-group">
                        <label for="location_pincode">Postal Code</label>
                        <input type="text" id="location_pincode" name="location_pincode" placeholder="10001" value="<?php echo htmlspecialchars($event['location_pincode']); ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" id="start_datetime" name="start_datetime" value="<?php echo date('Y-m-d\TH:i', strtotime($event['start_datetime'])); ?>" required>
            </div>
            
            <div class="form-group">
                <label for="end_datetime">End Date & Time</label>
                <input type="datetime-local" id="end_datetime" name="end_datetime" value="<?php echo date('Y-m-d\TH:i', strtotime($event['end_datetime'])); ?>" required>
            </div>
